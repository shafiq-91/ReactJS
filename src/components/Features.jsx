        ...
        <>
            <div className="p-4 flex flex-wrap items-center justify-center gap-5 h-auto">
                {cards.map((card, index) => (
                    <div
                        key={index}
                        className="feature-card flex flex-col max-w-sm md:max-w-md"
                    >
                        <img src={card.image} alt={card.title} className="p-2 w-16 md:w-20" />
                        <h3 className="text-[20px] md:text-[24px] p-2">{card.title}</h3>
